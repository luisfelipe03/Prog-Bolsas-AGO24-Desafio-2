...
// Função de seed
export const seedSqlite = () => {
    db.serialize(() => {
        // Consulta para checar se já existem dados na tabela
        const checkDataQuery = "SELECT COUNT(*) as count FROM Store";

        db.get(checkDataQuery, (err, row: CountResult) => {
            if (err) {
                logger.error(
            ...
            if (row.count === 0) {
                logger.info("Iniciando o seed de dados...");
                ...
                storesSeedData.forEach((store) => {
                    db.run(
                        ...
                        (err) => {
                            if (err) {
                                logger.error(
                                    "Erro ao inserir dados de seed:",
                                    err.message
                                );
                            } else {
                                logger.info(
                                    `Dados inseridos para a loja ${store.name}`
                                );
                            }
                ...
                logger.info(
                    "Seed já foi executado anteriormente. Dados já existem."
                );
            }
        });
    });
